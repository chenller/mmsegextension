...
from mmcv.cnn import Conv2d, build_plugin_layer, ConvModule, build_norm_layer
from mmdet.models.task_modules import AssignResult, MaskSamplingResult, BaseSampler
from mmengine.model.weight_init import caffe2_xavier_init, xavier_init, normal_init
...
from mmsegextension.registry import MODELS as HEADS
...
from mmseg.models.utils.point_sample import get_uncertain_point_coords_with_randomness
...
class MaskHungarianAssigner(BaseAssigner):
    ...
    def __init__(self,
        ...
        cls_cost.pop('type'), mask_cost.pop('type'), dice_cost.pop('type'),
        self.cls_cost = ClassificationCost(**cls_cost)
        self.mask_cost = CrossEntropyLossCost(**mask_cost)
...
# copy from 'https://github.com/czczup/ViT-Adapter/blob/main/segmentation/mmseg_custom/models/decode_heads/mask2former_head.py'
class Mask2FormerHeadGithubViTAdapter(BaseDecodeHead):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(Mask2FormerHeadGithubViTAdapter, self).__init__(
        ...
        loss_cls.pop('type'), loss_mask.pop('type'), loss_dice.pop('type')
        self.loss_cls = CrossEntropyLoss(**loss_cls)
        self.loss_mask = CrossEntropyLoss(**loss_mask)

        self.loss_dice = DiceLoss(**loss_dice)
    ...
    #          gt_masks_list, img_metas):
    def calculate_loss(self, all_cls_scores, all_mask_preds, gt_labels_list,
                       gt_masks_list, img_metas):
        """Loss function.
...
class AdapterMask2FormerHead(Mask2FormerHeadGithubViTAdapter):
    """Implements the Mask2Former head.
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: SampleList,
        ...
        batch_gt_instances, batch_img_metas = self._seg_data_to_instance_data(
            batch_data_samples)
        batch_gt_label, batch_gt_mask = [], []
        for gt_instances in batch_gt_instances:
            batch_gt_label.append(gt_instances.labels)
            batch_gt_mask.append(gt_instances.masks)
        # forward
        all_cls_scores, all_mask_preds = self(x, batch_data_samples)

        # loss
        losses = self.calculate_loss(all_cls_scores, all_mask_preds,
                                     batch_gt_label, batch_gt_mask, batch_img_metas)
